...
if(!/^[A-Za-z ]+$/.test(Stuname.value)){
    ...
    if(!/^[0-9a-zA-Z]+$/.test(StundentClass.value)){
        ShowError(StundentClass, "Please Enter the Numbers only");
        return;
    }
    if(!/^[0-9]+$/.test(StudentRoll.value)){
        ShowError(StudentRoll, "Please Enter the Numbers only");
        return;
    }
...
renderTable()
toggleTable(hasdata)
btn.addEventListener('click', ShowStudentData)
